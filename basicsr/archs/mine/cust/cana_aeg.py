...
class CUSTAttention(nn.Module):
    ...
    def forward(self, x):
        ...
        with torch.no_grad():
            ...
            low_freq = F.interpolate(low_res, size=(H, W), mode='bilinear')
            hf_error = (x - low_freq).abs() 
            err_grouped, _, _ = self.window_group_partition(hf_error)
            proto = (alpha * x_grouped.detach() + (1 - alpha) * err_grouped.detach()).mean(dim=3)  # [B, num_group, group_size, c]
