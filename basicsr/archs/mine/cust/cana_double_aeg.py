...
f"""
cana_double_aeg.py에서
추가로 cana에도 aeg를 /2, /4로 제공.
"""
...
class CUSTAttention(nn.Module):
    def __init__(self, 
        ...
        # error-similarity scaler
        self.alpha_vec = nn.Parameter(torch.ones(1,1,1,dim))
    ...
    def window_group_partition(self, x):
        ...
        gh, gw = H_pad // target_unit, W_pad // target_unit # 각 h, w 별 그룹 개수

        ########################################################
        # Partition and Grouping 
        # [B C (gh gs ws) (gw gs ws)] 8차원으로 한 번에 쪼개고
        # [B gh gw gs gs ws ws C]로 이동.
        ########################################################
    ...
    def window_group_reverse(self, x, original_shape, padded_size):
        ...
        ##########################################################
        # 8차원으로 복원 후 재배치([b, c, (gh gs ws), (gw gs ws)])
        # 이후 [B C H_pad W_pad]로 복구
        ##########################################################
    ...
    def forward(self, x):
        ...
        # 이후 err 계산, 역시 다시금 window_group_partition
        ###################
        x_grouped, pad_h, pad_w = self.window_group_partition(x)
        
        ### error grouping
        alpha = self.alpha_vec.clamp(0, 1)
        
        with torch.no_grad():
            x_d2 = F.adaptive_avg_pool2d(x, (H // 2, W // 2))
            x_u2 = F.interpolate(x_d2, size=(H, W), mode='bilinear')
            err2 = (x - x_u2).abs()
            
            x_d4 = F.adaptive_avg_pool2d(x_d2, (H // 4, W // 4))
            x_u4 = F.interpolate(x_d4, size=(H, W), mode='bilinear')
            err4 = (x_u2 - x_u4).abs() # 또는 (x_u2 - x_u4).abs()
            
            err2_grouped, _, _ = self.window_group_partition(err2)
            err4_grouped, _, _ = self.window_group_partition(err4)
            alpha = torch.sigmoid(self.alpha_vec)
            
            proto_err = (err2_grouped + err4_grouped) * 0.5 
            proto = (alpha * x_grouped.detach() + (1 - alpha) * proto_err).mean(dim=3)
        
        #####################################
        # 유사도 계산 및 다른 패치와 엮기 계산
        #####################################
        sim = torch.einsum('b g w p c, b g k c -> b g w p k', x_grouped.detach(), proto)   # [B, ng, gs, ws, gs]
...
class Low_to_high_MS_v2(nn.Module):
    def __init__(self, dim):
        ...
        # 1. 오차 정제 레이어 (오차 전체를 보고 넓은 맥락을 파악)
        self.error_refiner = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, padding=2, dilation=2, groups=dim, bias=False),
            nn.GELU(),
            nn.Conv2d(dim, dim, 1)
        ...
        # 2. 멀티스케일 통합 게이트 (1번 코드의 gate_gen 확장)
        # err2와 err4의 abs 에너지를 모두 보고, 최종적으로 어디를 보정할지 맵을 생성
        self.gate_gen = nn.Sequential(
    ...
    def forward(self, x):
        ...
        err4 = x_u2 - x_u4 # 혹은 사용자님의 아이디어인 x - x_u4
        
        # 1. Refiner는 두 오차를 합친 '전체 오차'의 맥락을 먼저 정교하게 다듬습니다.
        refined_error = self.error_refiner(err2 + err4)
        
        # 2. Gate는 두 스케일의 에너지를 모두 참조하여 "보정할 위치"를 최종적으로 찍어줍니다.
        error_energies = torch.cat([err2.abs(), err4.abs()], dim=1)
        spatial_gate = self.gate_gen(error_energies)
        
        # 3. 최종 결합: (가공된 오차) x (위치 마스크)
        return x + (self.scale * refined_error * spatial_gate)
