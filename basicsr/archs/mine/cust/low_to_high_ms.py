...
class Low_to_high(nn.Module):
    ...
    def forward(self, x):
        ...
        ### abs -> gating
        error_energy = err2.abs() + err4.abs() 
        sel = self.scale_selector(error_energy)
        
        w2 = sel[:, 0:1, :, :]
        w4 = sel[:, 1:2, :, :]
        
        combined_error = (err2 * w2) + (err4 * w4)
        
        return x + (self.scale * self.error_refiner(combined_error))
  

## LHT_ms_v1_b16
class Low_to_high_MS_v1(nn.Module):
    def __init__(self, dim):
        super().__init__()
        
        # 1. 통합 오차 정제 레이어 (성공했던 Dilation=2 유지)
        self.error_refiner = nn.Sequential(
            nn.Conv2d(dim, dim, kernel_size=3, padding=2, dilation=2, groups=dim, bias=False),
            nn.GELU(),
            nn.Conv2d(dim, dim, 1)
        ...
        # 2-1. 1/2 scale gating
        self.gate_gen2 = nn.Sequential(
            nn.Conv2d(dim, 1, kernel_size=1),
            nn.Sigmoid()
        )
        # 2-2. 1/4 scale gating
        self.gate_gen4 = nn.Sequential(
            nn.Conv2d(dim, 1, kernel_size=1),
            nn.Sigmoid()
        )
    ...
    def forward(self, x):
        ...
        # gate generating
        gate2 = self.gate_gen2(err2.abs())
        gate4 = self.gate_gen4(err4.abs())
        
        combined_error = (err2 * gate2) + (err4 * gate4)
        
        # refine
        refined_error = self.error_refiner(combined_error)
        
        return x + (self.scale * refined_error)
