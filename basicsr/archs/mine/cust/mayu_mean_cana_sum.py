...
f"""
mayu_win_cana_sum.py

mayu - cana 방식
mayu는 그룹 내에서의 self-attn

mayu는 현재 2가지 방식을 고민중이며,
하나는 x_mean을 self-attn하는 방식(이름은 mean으로 짓는다)
또는 x_grouped의 ws와 group(=win의 mean) 간 cross-attn하는 방식(이름은 cross로 짓는다)

mayu와 cana를 어떻게 이을지도 고민중이다.
1) mayu - cana  () : 없음
2) cana - mayu  (seq)
3) cana -+ - conv@11
   mayu -|          (sum)
...
class CUSTAttention(nn.Module):
    def __init__(self, 
        ...
        ### MAYU algorithm
        self.mayu_pe = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.mayu_qkv = nn.Linear(dim, hidden_dim * 3)
        self.mayu_proj = nn.Linear(dim, dim)
        self.mayu_gate = nn.Linear(dim, dim)
    ...
    def mayu(self, x_grouped, x_mean):
        f"""
        x_grouped : [B, num_group, group_size, win_size, c]
        x_mean : [B, num_group, group_size, c]
        gss : (gs)**2
        wss : (ws)**2
        """
        B, ng, gss, c = x_mean.shape
        _, _, _, wss, _ = x_grouped.shape
        
        # st()
        z = self.act(self.mayu_gate(x_mean))

        qkv = self.mayu_qkv(x_mean) 
        q, k, v = torch.chunk(qkv, 3, dim=-1) # [B, ng, 81, c]

        pe = rearrange(q, 'b ng (h w) c -> (b ng) c h w', h=self.group_size, w=self.group_size)
        pe = self.mayu_pe(pe)
        pe = rearrange(pe, '(b ng) c h w -> b ng (h w) c', b=B, ng=ng)
        
        attn = (q @ k.transpose(-1, -2)) * self.scale
        attn = attn.softmax(dim=-1)
        out = (attn @ v) + pe
        out = self.mayu_proj(out * z)
    ...
    def forward(self, x):
        ...
        # 유사도 계산
        #############  
        x_mean = x_grouped.detach().mean(dim=3) # [B, num_group, group_size, c]
        
        ########################
        # MAYU - 그룹 내 Attn
        ########################
        mayu_out = self.mayu(x_grouped, x_mean)

        ########################
        # CANA - 다른 패치와 엮기
        ########################
        sim = torch.einsum('b g w p c, b g k c -> b g w p k', x_grouped.detach(), x_mean)   # [B, ng, gs, ws, gs]
        cana_out = self.cana(x_grouped, sim)
        cana_out = cana_out + mayu_out.unsqueeze(-2)
...
class CUSTBlock(nn.Module):
    ...
    def forward(self, x):
        ...
        # 1. Attention (Pre-Norm & Residual)
        x = x + self.attn(self.norm1(x))
        
        # 2. FFN (Pre-Norm & Residual)
        shortcut = x
        x = self.norm2(x)
...
class Low_to_high(nn.Module):
    ...
    def forward(self, x):
        ...
        return x + (self.scale * self.error_refiner(hf_error))
