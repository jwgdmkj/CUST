...
class AddLinearAttention(nn.Module):
    ...
    def __init__(self, dim):
        ...
        # [핵심] 학습 가능한 변조 파라미터 (Modulation Parameters)
        # 초기값을 0으로 설정하여, 초기에는 modulation = exp(0) = 1.0 이 되도록 함.
        # 즉, 학습 초기에는 MALA 원본 수식과 100% 동일하게 동작함.
        self.mod_scale = nn.Parameter(torch.zeros(1, 1, 1))
        self.mod_bias = nn.Parameter(torch.zeros(1, 1, 1))
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor):
        ...
        ######### scaler #####################
        # 1. Base Scale (MALA의 강력한 Prior)
        # "z가 크면 스케일을 줄여라"라는 물리적 법칙을 강제 주입
        base_scale = base_scale = 1.0 + 1.0 / (z + 1e-6)
        
        # 2. Learnable Modulation (잔차 학습)
        # 네트워크가 필요에 따라 곡선을 미세 조정(Tuning)하도록 허용
        # exp를 쓰는 이유: 곱셈 연산(scaling)에서 양수 조건을 유지하고 미분 가능하기 때문
        modulation = torch.exp(self.mod_scale * z + self.mod_bias)
        
        # 3. Final Scale
        # MALA 수식 * 보정값
        final_scale = base_scale * modulation
        ######################################
        ...
        res = q @ kv * final_scale - z * v.mean(dim=2, keepdim=True)
