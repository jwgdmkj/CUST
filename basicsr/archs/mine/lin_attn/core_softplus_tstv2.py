...
class AddLinearAttention(nn.Module):
    ...
    def __init__(self, dim):
        ...
        self.a_raw = nn.Parameter(torch.zeros(1, 1, 1))
        self.b_raw = nn.Parameter(torch.zeros(1, 1, 1))
        self.c_raw = nn.Parameter(torch.zeros(1, 1, 1))
        self.d_raw = nn.Parameter(torch.zeros(1, 1, 1))
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor):
        ...
        a = 0.1 * F.softplus(self.a_raw)                          # >= 0
        b = 0.1 * torch.tanh(self.b_raw)
        beta = beta = base_scale * torch.exp(a * z + b)                  # (B, h, N, 1)
        
        # --- γ도 함께 조정 (bias correction term) ---
        c = 0.1 * F.softplus(self.c_raw)
        d = 0.1 * torch.tanh(self.d_raw)

        gamma = z * torch.exp(c * z + d)                          # (B, h, N, 1)
        ######################################
        ...
        res = q @ kv * beta - gamma * v.mean(dim=2, keepdim=True)
