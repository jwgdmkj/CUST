...
class my_mala(nn.Module):
    """
    - Q/K/V + DWConv positional encoding
    - Magnitude-aware Linear Attention (MALA)
    - Local conv branch + gating fusion
    입력/출력: (B, C, H, W), C = dim
    """
    def __init__(self, dim, num_heads):
        ...
        self.dwc_pe = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor):
        ...
        # Local branch (high-rank local stream)
        local_feat = self.local_mixer(v)      # (B, C, H, W)

        # (B, C, H, W) -> (B, h, N, d)
        q_mat = rearrange(q, 'b (h d) hgt wgt -> b h (hgt wgt) d',
                          h=self.num_heads)
        k_mat = rearrange(k, 'b (h d) hgt wgt -> b h (hgt wgt) d',
                          h=self.num_heads)
        v_mat = rearrange(v, 'b (h d) hgt wgt -> b h (hgt wgt) d',
                          h=self.num_heads)

        ####################################
        # 2. ELU and make z (scaling factor)
        ####################################
        q_act = self.elu(q_mat) + 1.0
        k_act = self.elu(k_mat) + 1.0

        # Normalizer Z  (B, h, N, 1)
        k_mean = k_act.mean(dim=-2, keepdim=True)  # (B, h, 1, d)
        z = torch.matmul(q_act, k_mean.transpose(-2, -1)) * self.scale

        ################################
        # 3. KV buffer and global output
        ################################
        scale_factor = (self.scale / N) ** 0.5
        # KV: (B, h, d, d)
        kv = torch.matmul(
            (k_act * scale_factor).transpose(-2, -1),
            (v_mat * scale_factor)
        )

        # numerator: (B, h, N, d)
        numerator = torch.matmul(q_act, kv)
        attn_out = numerator * (1.0 + 1.0 / (z + 1e-6))

        # Bias correction
        v_mean = v_mat.mean(dim=-2, keepdim=True)      # (B, h, 1, d)
        global_feat = attn_out - (z * v_mean)         # (B, h, N, d)

        #################################
        # 4. reshape + gating + fusion
        #################################
        global_feat = global_feat.permute(0,1,3,2).view(B, -1, H, W).contiguous()

        # O를 simple gate로 사용 (시그모이드)
        gate_o = torch.sigmoid(o)
        global_feat = global_feat * gate_o

        # global + local 을 더해서 proj 한 번만 적용
        fused = global_feat + local_feat                   # (B, C, H, W)
