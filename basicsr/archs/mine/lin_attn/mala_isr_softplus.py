...
class my_mala(nn.Module):
    def __init__(self, dim, num_heads):
        ...
        # -----------------------------------------------------------
        # [핵심 변경] Learnable Magnitude Mapping Parameters
        # 각 Head별로 다른 곡선을 학습하도록 (num_heads, 1, 1) 형태
        # s = Softplus(a*z + b)  -> Main term scaling
        # t = Softplus(c*z + d)  -> Subtraction term scaling
        # -----------------------------------------------------------
        self.param_a = nn.Parameter(torch.zeros(1, num_heads, 1, 1))
        self.param_b = nn.Parameter(torch.ones(1, num_heads, 1, 1) * 2.0) # 초기값 조절
        
        self.param_c = nn.Parameter(torch.zeros(1, num_heads, 1, 1))
        self.param_d = nn.Parameter(torch.zeros(1, num_heads, 1, 1))
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor):
        ...
        # 3. Scaling and KV
        ################################
        s = F.softplus(self.param_a * z + self.param_b) # Main Scale
        t = F.softplus(self.param_c * z + self.param_d) # Centering Scale
        
        k_act = k_act * (self.scale / N) ** 0.5
        v_mat = v_mat * (self.scale / N) ** 0.5
        
        kv = k_act.transpose(-2, -1) @ v_mat
        
        ################################
        # 4. MM query and normalization
        # res = (q @ kv) * s - (z * v_mean) * t  <-- 이런 형태
        ################################
        term1 = (q_act @ kv) * s
        
        v_mean = v_mat.mean(dim=-2, keepdim=True)
        # z * v_mean은 "Bias Correction" 항임. 이것도 학습 가능하게 t를 곱함
        # 혹은 논문 제안처럼 단순하게 t * v_mean 만 할 수도 있음.
        # 여기서는 MALA 수식 형태를 유지하며 파라미터화:
        term2 = z * v_mean * t 

        res = term1 - term2
        
        res = rearrange(res, 'b n (h w) d -> b (n d) h w', h=H, w=W)
        return self.fusion(res * o)
