...
from pdb import set_trace as st
f"""
MALA_last_conv_expand.py

dif_branch_outer_gate_expand의 Conv 부분을 MALA로 변형.
LMLT와 같은 Branch 구조 제거. 그냥 head 4개 버전.

업스케일링 후 곱할때, 마지막 브랜치에도 게이트를 곱하고, sigmoid로 대체
"""
...
class HighFreqConv(nn.Module):
    def __init__(self, dim):
        ...
        # x proj and high-filter conv
        # x = x+conv33+conv55 or x=conv77
        self.x_proj = nn.Linear(dim, dim)
        self.conv33 = nn.Conv2d(dim, dim, 3, 1, 1, groups=dim)
        self.conv55 = nn.Conv2d(dim, dim, 5, 1, 2, groups=dim)
        # self.conv77 = nn.Conv2d(dim, dim, 7, 1, 3, groups=dim)
    ...
    def forward(self, x):
        f"""
        x - linear - conv33+conv55+x --- x - proj
          | - linear(gate) ------act-----|
        """
        B, chan, H, W = x.shape
        ...
        z = self.act(self.z_proj(x))
        
        # high-freq conv
        x_proj = x_proj + self.conv33(x_proj) + self.conv55(x_proj)
        x_proj = x_proj.permute(0,2,3,1).contiguous()  # [B H W C]

        # out_proj with gate
        x = self.out_proj(x_proj * self.act(z))
...
class LocalMixer(nn.Module):
    """
    ISR용 local branch.
    작은 NAF-style 블록: PWConv -> GELU -> DWConv -> PWConv
    """
    def __init__(self, dim, expansion=2):
        super().__init__()
        hidden_dim = int(dim * expansion)
        self.pw1 = nn.Conv2d(dim, hidden_dim, 1)
        self.dw = nn.Conv2d(hidden_dim, hidden_dim,
                            kernel_size=3, padding=1,
                            groups=hidden_dim)
        self.act = nn.GELU()
        self.pw2 = nn.Conv2d(hidden_dim, dim, 1)

    def forward(self, x):
        f"""
        x - conv11 - dwconv - gelu - conv11
                            |------|
        """                 
        x = self.pw1(x)
        dwx = self.dw(x)
...
class my_mala(nn.Module):
    def __init__(self, dim, num_heads):
        ...
        # High-Rank Local Stream
        # self.local_mixer = LocalMixer(dim, expansion=1.5)
        self.local_mixer_small = nn.Conv2d(dim, dim, 5, 1, 2, groups=dim)
    ...
    def forward(self, x: torch.Tensor, sin: torch.Tensor, cos: torch.Tensor):
        '''
        RoPE 제거 -> sin/cos를 사용할 경우, 위치 불변성이 중요한 SR에서는 성능의 저하가 발생하거나, 불필요
        할수 있다고 함. 따라서 DConv@nn과 같은 암시적 위치정보(cnn의 귀납적 편향)로 변경.

        High-Rank Local Stream 추가를 통해, V 랭크 보존.
        '''
        ...
        local_feat = self.local_mixer_small(v)
        
        q_mat = rearrange(q, 'b (n d) h w -> b n (h w) d', n=self.num_heads)
        k_mat = rearrange(k, 'b (n d) h w -> b n (h w) d', n=self.num_heads)
        v_mat = rearrange(v, 'b (n d) h w -> b n (h w) d', n=self.num_heads)

        ####################################
        # 2. ELU and make z(sclaing factor)
        ####################################
        q_act = self.elu(q_mat) + 1.0
        k_act = self.elu(k_mat) + 1.0
        # q_act = self.polynomial_kernel(q_mat)
        # k_act = self.polynomial_kernel(k_mat)

        # Normalizer Z
        k_mean = k_act.mean(dim=-2, keepdim=True)
        z = q_act @ k_mean.transpose(-2, -1) * self.scale
        
        ################################
        # 3. kv and output
        ################################
        scale_factor = (self.scale / N) ** 0.5
        kv = (k_act * scale_factor).transpose(-2, -1) @ (v_mat * scale_factor)
        
        numerator = q_act @ kv
        attn_out = numerator * (1 + 1/(z + 1e-6))
        
        # Bias Correction
        v_mean = v_mat.mean(dim=-2, keepdim=True)
        global_feat = attn_out - (z * v_mean)
        
        #################################
        # 4. High-Rank Local Stream
        #################################
        global_feat = rearrange(global_feat, 'b n (h w) d -> b (n d) h w', h=H, w=W)
        global_feat = global_feat * o
        
        
        return self.fusion(global_feat + local_feat)
    
        # out = self.fusion(torch.cat([global_feat, local_feat], dim=1))
        # return out
...
class GBC(nn.Module):
    def __init__(self, 
        ...
        self.gbc = nn.ModuleList([
            my_mala(self.branch_dim[i],
                               num_heads=num_heads,
                               )
            for i in range(self.n_levels - 1)])
        self.gbc.append(HighFreqConv(self.branch_dim[n_levels-1]))
        ...
        for i in range(self.n_levels):
            ...
            if i < self.n_levels - 1 :  # except last branch
                ...
                * self.sigmoid(gate[:, gate_idx:gate_idx+self.branch_dim[i]])
                ...
                branch = self.gbc[i](downsized_feats[i])   
...
class BasicBlock(nn.Module):
    def __init__(self, 
        ...
        self.blqm = GBC(dim, 
                        ...
                        last_branch_conv=last_branch_conv, 
...
if __name__== '__main__':
    ...
    branch_dim = [8, 12, 16, 50]
    n_levels = len(branch_dim)
    num_heads = 1
