...
class PCFN(nn.Module):
    ...
    """
    def __init__(self, dim, growth_rate=2.0, p_rate=0.20):
        super().__init__()
        hidden_dim = int(dim * growth_rate)
...
class LHSBv2(nn.Module):
    def __init__(self,
        ...
        if self.use_aggr_expand:    # Aggr Skip Connection
            self.alpha = nn.Parameter(1e-4 * torch.ones(1, dim), requires_grad=True)
    ...
    def forward(self, x, aggr=None):
        ...
        ## aggregation
        ####################################
        if self.use_aggr_expand:
            # alpha = torch.sigmoid(self.alpha.unsqueeze(-1).unsqueeze(-1))   # [B, 1, 1, C]
            alpha = self.alpha.unsqueeze(-1).unsqueeze(-1)   # [B, 1, 1, C]
            out = self.aggr(alpha * aggr + (1-alpha) * torch.cat(out, dim=1))
            aggr_skip = out
        else :
...
class AttBlock(nn.Module):
    def __init__(self, 
        ...
        self.lhsb = LHSBv2(dim, 
                         ...
                         window_size=window_size,
                         idx=idx,
                         use_aggr_expand=use_aggr_expand,) 
    ...
    def forward(self, x, aggr=None):
        ...
        ## LHSB
        if self.use_aggr_expand:
            x_out, aggr = self.lhsb(self.norm1(x), aggr)
            x = x_out + x
        else :
...
class LMLT(nn.Module):
    def __init__(self, 
        ...
        self.feats = nn.ModuleList([AttBlock(dim, 
                                            ffn_scale, 
                                            drop=drop_rate,
