...
aggr_out_fusion.py
기존 aggr_fusion은 aggr 결과를 다음 block의 aggr에 더해주었다면,
이번에는 최종 out 결과를 aggr에 더해줌.
"""
...
class LHSBv2(nn.Module):
    ...
    def forward(self, x, aggr=None):
        ...
        if self.use_aggr_expand:
            ...
            out = self.aggr(self.alpha[0] * aggr + self.alpha[1] * torch.cat(out, dim=1))
        else :
