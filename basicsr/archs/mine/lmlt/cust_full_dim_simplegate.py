...
class LayerNorm(nn.Module):
    def __init__(self, normalized_shape, eps=1e-6, data_format="channels_first"): 
        ...
        if data_format == "channels_last": 
            ...
            # [핵심 변경] channels_first일 때는 GroupNorm(1, C)를 씁니다. 
            # # 이것은 LayerNorm과 수학적으로 완전히 같지만, 
            # # 수동 연산이 아니라 최적화된 커널을 써서 메모리를 훨씬 적게 먹습니다. 
            self.norm = nn.GroupNorm(num_groups=1, num_channels=normalized_shape, eps=eps) 
            
    def forward(self, x): 
        if self.data_format == "channels_last": 
            return self.norm(x) 
        else: # GroupNorm은 입력이 [B, C, H, W]일 때 잘 작동합니다. 
            return self.norm(x)
...
class MainBlock(nn.Module):
    def __init__(self, 
        ...
        ### Feedforward layer
        self.ffn2 = GatedFFN(dim, growth_rate=ffn_scale) 
