...
cust를 full_dim을 쓰고, multiaggregation사용
중간 ffn 없음.
"""
...
class newGatedFFN(nn.Module):
    def __init__(self, dim, growth_rate=2.0, p_rate=0.25):
        ...
        # 2. Partial Processing (Standard Conv 3x3)
        # - groups=1 (Standard Conv): 채널 간 정보를 빡빡하게 섞음 (성능 up, 연산량 up)
        # - dilation=2: 같은 3x3 파라미터로 5x5 영역을 커버함 (Context up)
        self.conv_1 = nn.Conv2d(p_dim, p_dim, 3, 1, padding=2, dilation=2) 
    ...
    def channel_shuffle(self, x, groups):
        # ShuffleNet의 채널 셔플링 구현
        b, c, h, w = x.size()
        # 채널을 groups로 나누고 reshape -> transpose -> flatten
    ...
    def forward(self, x):
        ...
        # 3. Spatial Mixing (Standard Conv on Partial channels)
        # 여기서 일반 Conv가 사용되므로 x1 내부의 채널끼리는 완벽하게 섞입니다.
        x1 = self.act(self.conv_1(x1))
        
        # 4. Concat
        x = torch.cat([x1, x2], dim=1)
        
        # 5. ★ Channel Shuffle (핵심)
        # p_dim과 나머지 채널을 섞어주어 다음 레이어에서 정보가 고르게 퍼지게 함
        # 그룹 수는 2로 설정하여 (처리된 것 / 안 된 것)을 골고루 섞음
...
class CUST(nn.Module):
    ...
    def cluster_cat_kv(self, x_grouped, sim):
        ...
        ###########################################################################
        # B) Key/val(현 청크 + 다음 청크)
        # pad_x = torch.zeros(B*ng, cs, chan, device=device)
        # pad_x = torch.cat([x_sorted, pad_x], dim=1) # [B*ng, gs*ws + 64, chan]
        # pad_id = torch.full((B*ng, cs), -1, device=device)
        # pad_id = torch.cat([id_sorted, pad_id], dim=1)  # [B*ng, gs*ws+64]
...
class AttnBlock(nn.Module):
    def __init__(self,
        ...
        self.norm = LayerNorm_old(dim)
        
        ### Spatial Weighting ###
        self.gmlt = nn.ModuleList([
            GMLT(self.branch_dim[i],
                           window_size=window_size,
                           attn_drop=attn_drop,
                           proj_drop=proj_drop,
                           down_scale=2**i)
    ...
    def forward(self, x, aggr=None):
        ...
        for i in range(self.n_levels):
            if i < self.n_levels - 1 :  # except last 
                branch = self.gmlt[i](downsized_feats[i])      
...
class MainBlock(nn.Module):
    def __init__(self, 
        ...
        ### Feedforward layer
        self.ffn = GatedFFN(dim, growth_rate=ffn_scale) 

    def forward(self, x):
        # x = self.cust(self.norm1(x)) + x
        # x = self.ffn(self.norm2(x)) + x
        
        x = self.attns(self.norm2(x)) + x
