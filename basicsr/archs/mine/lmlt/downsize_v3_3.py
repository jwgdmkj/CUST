...
v3_3 : 다운사이즈를 시리즈 별로 다르게(첫 헤드는 없음, 두번째는 2, 세번째는 4, 마지막은 8) 
       이 때, 양옆 32개를 다음 kv에 사용.

v3_5 : 양옆 32개를 다음 kv에 사용(다음 64개가 아니라).

v4 : 그룹핑 글로벌 어텐션이 아닌, 풀링형 글로벌 어텐션. 이후 넘버링은 같은 방식으로.
"""
...
class InterWindowAttn(nn.Module):
    ...
    def cluster_cat_kv(self, x_grouped, sim, gate_z=None):
        f"""
        다음 청크를 키/밸류에 추가. 
    ...
    def forward(self, x):
        ...
        if self.use_gate_like:
            z = self.act_gate(self.z(x.permute(0,2,3,1)))  # [B, H, W, C]
