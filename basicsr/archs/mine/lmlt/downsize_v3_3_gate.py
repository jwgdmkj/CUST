...
그룹핑 버전
v3 : x에 그룹핑 글로벌 어텐션 후 청킹
v3_1 : 그룹핑 글로벌 + 그냥
v3_2 : cat(그룹핑 글로벌, 그냥)
v3_3 : 다운사이즈를 시리즈 별로 다르게(첫 헤드는 없음, 두번째는 2, 세번째는 4, 마지막은 8) 
       이 때, 양옆 32개를 다음 kv에 사용. 추가로, 게이트-라이크 도입.
       
v3_gate : v3_3을 기반으로 하되, 다운사이즈를 (없음-2-4-8)
          파라미터 개수가 감소하므로, global_attn에 global_like gate 추가
"""
...
class InterWindowAttn(nn.Module):
    def __init__(self, 
        ...
        f"""
        window_size : 풀링 기준
        group_size : 몇 개의 윈도우를 그룹핑 할 것인가(제곱)
        """
        
        ### var
        self.window_size = window_size # 8/4/2
        self.group_size = group_size   # 9
        self.scale = dim ** -0.5
    ...
    def cluster_cat_kv(self, x_grouped, sim):
        ...
        ### mamba_like gating
        if self.use_gate_like:
            z = self.act_gate(self.z_proj(x_grouped.view(B, ng, gs*ws, chan)))   # [B, ng, gs*ws, C]
        ...
        # key/ val : [이전 절반 청크 + 현재 청크 + 다음 절반 청크]
        ###################################################
        ...
        pad_id = torch.full((B*ng, cs//2), -1, device=x_grouped.device)
        ...
        # False인 부분(ID 불일치)을 1e-4로 마스킹 -> 불순물 완벽 차단
        min_val = -1e4
...
class LHSBv2(nn.Module):
    def __init__(self,
        ...
        self.interattns = nn.ModuleList([
            InterWindowAttn(dim=self.branch_dim[i+1],
                            window_size=2**(i+1),)
            for i in range(self.n_levels - 1)
        ])  # v3_3 버전: window_size를 (무)/2/4/8로, neighborattn과는 반대로
        ...
        # self.beta = nn.Parameter(torch.ones(dim) * 0.5)
    ...
    def forward(self, x, aggr=None):
        ...
        ## split & ngating
        ####################################
        xc = list(torch.split(x, self.branch_dim, dim=1))
        SA_before_idx = None
        out = []
        ...
        for i in range(self.n_levels - 1):
            # st()
            xc[i+1] = xc[i+1] + self.interattns[i](xc[i+1])
