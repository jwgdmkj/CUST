...
v3_1 : 그룹핑 글로벌 + 그냥
v3_2 : cat(그룹핑 글로벌, 그냥)
v3_3 : 다운사이즈를 시리즈 별로 다르게(첫 헤드는 8, 두번째는 4, 세번째는 2, 마지막은 안함)
v3_4 : 추가로 레이어를 따로 빼서 ln-ds-ln-mlp-ds-lhsb-ln-mlp 형태로 변경
...
class InterWindowAttn(nn.Module):
    ...
    def cluster_cat_kv(self, x_grouped, sim):
        ...
        # False인 부분(ID 불일치)을 -inf로 마스킹 -> 불순물 완벽 차단
        min_val = -1e4
