...
v3_5 : 양옆 32개를 다음 kv에 사용(다음 64개가 아니라). 
       이를 따로 global_x로 둔 다음, 마지막에 alpha, beta로 std와 가중합.
...
class LHSBv2(nn.Module):
    def __init__(self,
        ...
        self.alpha = nn.Parameter(torch.ones(dim, 1, 1) * 0.5)
        self.beta = nn.Parameter(torch.ones(dim, 1, 1) * 0.5)
    ...
    def forward(self, x, aggr=None):
        ...
        st()
        out = self.act(self.alpha * out + self.beta * global_x) * x
