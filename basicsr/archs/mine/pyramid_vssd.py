...
class BlqMamba(nn.Module):
    ...
    def forward(self, x):
      ...
      ######################################
      # 1. channel-wise pyramid downsize
      ######################################
      xc = torch.split(x, self.branch_dim, dim=1)
      downsized_feats = []

      for i in range(self.n_levels):
        ds_ratio = self.n_levels - i - 1    # 3-2-1-0
        if ds_ratio > 0 :
          p_size = (H//2**ds_ratio, W//2**ds_ratio)
      ...
      ######################################
      # 2. VSSD architecture per feature
      ######################################
      for i in range(self.n_levels):
        # process vssd and insert original shape into output
        branch = self.mmb[i](downsized_feats[i])
        branch_original_shape = F.interpolate(branch, size=(H,W),
                                              mode='nearest')
