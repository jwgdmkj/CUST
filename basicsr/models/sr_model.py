...
class SRModel(BaseModel):
    ...
    def optimize_parameters(self, current_iter):
        ...
        # ------------------ 핵심: Grad Clip 및 로그 기록 ------------------ #
        if self.opt.get('use_grad_clip', False):
            # 1. Clip을 수행하고 원본 노름을 받습니다 (반환값은 float)
            grad_norm = torch.nn.utils.clip_grad_norm_(
                self.net_g.parameters(), 
                self.opt.get('grad_clip_max_norm', 0.1)
            )
            # 2. float을 Tensor로 변환하여 에러 방지 (중요!)
            loss_dict['grad_norm'] = torch.as_tensor(grad_norm, device=self.device)
        else:
            # Clip을 안 쓰더라도 수치 확인을 위해 함수 호출
            g_norm = get_grad_norm(self.net_g)
            loss_dict['grad_norm'] = torch.as_tensor(g_norm, device=self.device)
        # --------------------------------------------------------------- #
